...
class Halomuon(CommonFSQFramework.Core.ExampleProofReader.ExampleProofReader):
...
#        self.hist['hSector_Mean_RMS'].Fill( sector_mean, sector_rms )
        ...
        for i in xrange(0,16):
            if self.nEvents > 0:
                ...
                self.sect_mean[i] /= self.nEvents
                self.sect_RMS[i] = sqrt(self.sect_RMS[i]/self.nEvents - self.sect_mean[i]**2)
        ...
        for i in xrange(0,224):
            if self.nEvents>0:
               self.ch_mean[i] /= self.nEvents
               self.ch_RMS[i] = sqrt(self.ch_RMS[i]/self.nEvents - self.ch_mean[i]**2)
...
if __name__ == "__main__":
   ...
   # nWorkers = 1
    ...
    Halomuon.runAll(treeName="CastorTree",
           slaveParameters=slaveParams,
           sampleList=sampleList,
           maxFilesMC = maxFilesMC,
           maxFilesData = maxFilesData,
           nWorkers=nWorkers,
           outFile = "plotsHalomuon.root" )
