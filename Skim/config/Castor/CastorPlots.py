...
class CastorPlots(CommonFSQFramework.Core.ExampleProofReader.ExampleProofReader):
    def init( self, maxEvents = None):
        ...
        self.hist["hottest_tower_notHFAnd"] = ROOT.TH1D("hottest_tower_notHFAnd", "hottest_tower_notHFAnd", 500, -50, 2000)
        ...
        if (self.haveCastorDigi) :
            ...
            for isec in xrange(0, 16):
                hname = "hottest_tower_digi_{sec}_BptxPlus".format(sec=str(isec+1))
                self.hist[hname] =  ROOT.TH1D(hname, hname, 500, -50, 2000)
                hname = "hottest_tower_digi_em_{sec}_BptxPlus".format(sec=str(isec+1))
                self.hist[hname] =  ROOT.TH1D(hname, hname, 500, -50, 2000)
                hname = "hottest_tower_digi_had_{sec}_BptxPlus".format(sec=str(isec+1))
                self.hist[hname] =  ROOT.TH1D(hname, hname, 500, -50, 2000)

                hname = "hottest_tower_digi_{sec}_BptxMinus".format(sec=str(isec+1))
                self.hist[hname] =  ROOT.TH1D(hname, hname, 500, -50, 2000)
                hname = "hottest_tower_digi_em_{sec}_BptxMinus".format(sec=str(isec+1))
                self.hist[hname] =  ROOT.TH1D(hname, hname, 500, -50, 2000)
                hname = "hottest_tower_digi_had_{sec}_BptxMinus".format(sec=str(isec+1))
                self.hist[hname] =  ROOT.TH1D(hname, hname, 500, -50, 2000)

                hname = "hottest_tower_digi_{sec}_NotBptxOR".format(sec=str(isec+1))
                self.hist[hname] =  ROOT.TH1D(hname, hname, 500, -50, 2000)
                hname = "hottest_tower_digi_em_{sec}_NotBptxOR".format(sec=str(isec+1))
                self.hist[hname] =  ROOT.TH1D(hname, hname, 500, -50, 2000)
                hname = "hottest_tower_digi_had_{sec}_NotBptxOR".format(sec=str(isec+1))
                self.hist[hname] =  ROOT.TH1D(hname, hname, 500, -50, 2000)

        if (self.haveHF) :
            self.hist["HF_INEL_NotBptxOR"] =  ROOT.TH1D("HF_INEL_NotBptxOR", "HF_INEL_NotBptxOR", 500, -10, 500)
            self.hist["HF_NSD_NotBptxOR"] =  ROOT.TH1D("HF_NSD_NotBptxOR", "HF_NSD_NotBptxOR", 500, -10, 500)
            
            self.hist["HF_INEL_BptxPlus"] =  ROOT.TH1D("HF_INEL_BptxPlus", "HF_INEL_BptxPlus", 500, -10, 500)
            self.hist["HF_NSD_BptxPlus"] =  ROOT.TH1D("HF_NSD_BptxPlus", "HF_NSD_BptxPlus", 500, -10, 500)
            
            self.hist["HF_INEL_BptxMinus"] =  ROOT.TH1D("HF_INEL_BptxMinus", "HF_INEL_BptxMinus", 500, -10, 500)
            self.hist["HF_NSD_BptxMinus"] =  ROOT.TH1D("HF_NSD_BptxMinus", "HF_NSD_BptxMinus", 500, -10, 500)
        ...
        self.hist["castor_em_frac"] =  ROOT.TH2D("castor_em_frac", "castor_em_frac", 16, 0.5, 16.5, 20, 0, 1)
        
        self.hist["castor_hf"] =  ROOT.TH2D("castor_hf", "castor_hf", 100, 0, 500, 100, 0, 3000)
        self.hist["castor_hf_notHFOr"] =  ROOT.TH2D("castor_hf_notHFOr", "castor_hf_notHFOr", 100, 0, 100, 100, 0, 300)
    ...
    def analyze(self):
        ...
        if (self.haveL1) :
            for iL1 in xrange(self.minL1, self.maxL1):
        ...
        for ich in xrange(self.fChain.CastorRecHitEnergy.size()):
            isec = ich//14
            imod = ich%14
        ...
        if (self.haveCastorDigi) :
            for ich in xrange(self.fChain.CastorDigisADC_TS4.size()):
                isec = ich//14
                imod = ich%14
        ...
        digi_secsum_max_sec = [0.0] * 16
        digi_secsum_max_em_sec = [0.0] * 16
        digi_secsum_max_had_sec = [0.0] * 16
        n_sec = 0
        for isec in xrange(16):
            ...
            if (energy_secsum[isec]>0.8): # count towers above noise
                n_sec += 1
            ...
            digi_secsum_max_sec[isec] = max(digi_secsum_max_sec[isec], digi_secsum)
            digi_secsum_max_em_sec[isec] = max(digi_secsum_max_em_sec[isec], digi_secsum_em)
            digi_secsum_max_had_sec[isec] = max(digi_secsum_max_had_sec[isec], digi_secsum_had)
            
            self.hist["castor_tower"].Fill(energy_secsum[isec]) 
            energy_tot += energy_secsum[isec]
        
        if (n_sec>0 and min(hfp_tow_energy_max, hfm_tow_energy_max)<5) :
            self.hist["castor_em_frac"].Fill(n_sec, energy_secsum_max_em/energy_secsum_max)
        
        self.hist["castor_hf"].Fill(max(hfp_tow_energy_max, hfm_tow_energy_max), energy_secsum_max*0.32)
        if (min(hfp_tow_energy_max, hfm_tow_energy_max)<5) :
            self.hist["castor_hf_notHFOr"].Fill(max(hfp_tow_energy_max, hfm_tow_energy_max), energy_secsum_max*0.32)
        ...
        if (max(hfp_tow_energy_max, hfm_tow_energy_max)<5) :
            self.hist["hottest_tower_notHFAnd"].Fill(energy_secsum_max)
        ...
        if ((self.haveL1 and self.fChain.trgl1L1GTAlgo[112]) or
            (not self.haveL1 and self.fChain.HLTBptxPlusOnly)):
            ...
            if (self.haveCastorDigi) :
                ...
                for isec in xrange(0, 16):
                    self.hist["hottest_tower_digi_{sec}_BptxPlus".format(sec=str(isec+1))].Fill(digi_secsum_max_sec[isec])
                    self.hist["hottest_tower_digi_em_{sec}_BptxPlus".format(sec=str(isec+1))].Fill(digi_secsum_max_em_sec[isec])
                    self.hist["hottest_tower_digi_had_{sec}_BptxPlus".format(sec=str(isec+1))].Fill(digi_secsum_max_had_sec[isec])

            if (self.haveHF) :
                self.hist["HF_INEL_BptxPlus"].Fill(min(hfm_tow_energy_max, hfp_tow_energy_max))
                self.hist["HF_NSD_BptxPlus"].Fill(max(hfm_tow_energy_max, hfp_tow_energy_max))
                
        # check if L1_UnpairedBunchBptxMinus
        if ((self.haveL1 and self.fChain.trgl1L1GTAlgo[113]) or
            (not self.haveL1 and self.fChain.HLTBptxMinusOnly)): 
            ...
            if (self.haveCastorDigi) :
                ...
                for isec in xrange(0, 16):
                    self.hist["hottest_tower_digi_{sec}_BptxMinus".format(sec=str(isec+1))].Fill(digi_secsum_max_sec[isec])
                    self.hist["hottest_tower_digi_em_{sec}_BptxMinus".format(sec=str(isec+1))].Fill(digi_secsum_max_em_sec[isec])
                    self.hist["hottest_tower_digi_had_{sec}_BptxMinus".format(sec=str(isec+1))].Fill(digi_secsum_max_had_sec[isec])
                
            if (self.haveHF) :
                self.hist["HF_INEL_BptxMinus"].Fill(min(hfm_tow_energy_max, hfp_tow_energy_max))
                self.hist["HF_NSD_BptxMinus"].Fill(max(hfm_tow_energy_max, hfp_tow_energy_max))

        # check if L1_NotBptxOR
        if ((self.haveL1 and self.fChain.trgl1L1GTAlgo[103]) or
            (not self.haveL1 and self.fChain.HLTNotBptxOR)): 
            ...
            if ((BX>20 and BX<50) or
                ...
                if (self.haveCastorDigi) :
                    ...
                    for isec in xrange(0, 16):
                        self.hist["hottest_tower_digi_{sec}_NotBptxOR".format(sec=str(isec+1))].Fill(digi_secsum_max_sec[isec])
                        self.hist["hottest_tower_digi_em_{sec}_NotBptxOR".format(sec=str(isec+1))].Fill(digi_secsum_max_em_sec[isec])
                        self.hist["hottest_tower_digi_had_{sec}_NotBptxOR".format(sec=str(isec+1))].Fill(digi_secsum_max_had_sec[isec])

                if (self.haveHF) :
                    self.hist["HF_INEL_NotBptxOR"].Fill(min(hfm_tow_energy_max, hfp_tow_energy_max))
                    self.hist["HF_NSD_NotBptxOR"].Fill(max(hfm_tow_energy_max, hfp_tow_energy_max))
